...
export class CreateSecretsStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    config({
      path: join(__dirname, "../env", `${process.env.ENVIRONMENT}.env`),
    });
    ...
    if (
      ...
      new ssm.StringParameter(this, "ClientSecretParam", {
        ...
        stringValue: CLIENT_SECRET,
      });

      new ssm.StringParameter(this, "IssuerParam", {
        parameterName: "issuer",
        stringValue: ISSUER,
      });
    } else {
      console.log(
        "must define a environment file ie. local.env and set the name of it as an environment variable ie. export ENVIRONMENT=local"
      );
    }
  }
}
